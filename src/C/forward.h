...
static TTMERR scan(TTM* ttm, utf8** cp8p);
static TTMERR testfcnprefix(TTM* ttm, utf8** pcp8, enum FcnCallCases* pfcncase);
static TTMERR scan1(TTM* ttm, utf8** cp8p, int* ncpp);
static TTMERR collectargs(TTM* ttm, Frame** framep, utf8** cp8p);
static TTMERR exec(TTM* ttm, Frame* frame);
static TTMERR call(TTM* ttm, Frame* frame, utf8* body);
static TTMERR processfcn(TTM* ttm, utf8** pcp8);
static TTMERR printstring(TTM* ttm, const utf8* s8, FILE* output);
static const char* print2len(VString* vs);
static const char* printsubseq(VString* vs, size_t argstart , size_t argend);
static utf8* desegmark(const utf8* s8, size_t* pfinallen);
static utf8* decontrol(const utf8* s8, char* ctrls, size_t* pfinallen);
...
static void startupcommands(TTM* ttm);
static TTMERR execstring(TTM* ttm, const char* cmd);
static void lockup(TTM* ttm);
static TTMERR collectescaped(TTM* ttm, utf8** cp8p);
static const char* sv(Function* f);
static TTMERR peek(VString* vs, size_t n, utf8** pp);
static void initglobals();
static void usage(const char* msg);
static void readinput(TTM* ttm, const char* filename, VString* buf);
static void readfile(TTM* ttm, FILE* file, VString* buf);
...
static int fgetc8(TTM* ttm, FILE* f, utf8* cp8);
static void fpushbackc(TTM* ttm, utf8* cp8);
static int fputc8(TTM* ttm, const utf8* c8, FILE* f);
static long long getRunTime(void);
static int timeofday(struct timeval *tv);
static long long getRunTime(void);
static int timeofday(struct timeval *tv);
static void xprintf(const char* fmt,...);
static void vxprintf(const char* fmt, va_list ap);
...
#ifdef GDB
static void dumpdict0(struct HashTable* dict, int printvalues);
