...
# Takeover part of the UTF8 space to store segment marks and create marks
# Specifically the private use area (PUA) in the  Basic Multilingual Plane.
# This covers the two byte value range U+E000 thru U+F8FF.
# This range is split in two parts:
# segment marks: U+E000 thru u+E0FF
# create marks : U+E100 // only one create mark is used in TTM.

SEGMARK = 0xE000
CREATEMARK = 0xE100
MARKMASK = 0xFF00 # mask off specific mark index
INDEXMASK = 0x00FF # mask off index byte
...
CREATEFORMAT = "{0:04d}" # use during create instantiation
SEGFORMAT = "{0:02d}"    # use for segment|create escaping
...
# functions to set/clear/test flag marks
...
##################################################
# Segment and creation marks

# Return a char for the n'th segment mark (0 <= n < 256)
# The segment is encoded as two bytes: 0xE0, 0x00-0xFF.

def makesegmark(n):
  return chr(SEGMARK+(n & 0xFF))

# Ditto for creation marks
# The mark is encoded as two bytes: 0xE1, 0x00-0xFF.

def makecreatemark():
  return chr(CREATEMARK)

def issegmark(c):
  return ((ord(c) & MARKMASK) == SEGMARK)

def iscreatemark(c):
  return ((ord(c) & MARKMASK) == CREATEMARK)
...
class StringBuffer:
  ...
  def insert(self, src, offset=-1, n=-1):
    ...
    # insert 
    if isstring:
      x = src[start:(start + count)]  # may not be necessary
      newdata = list(x)
    else:
  ...
  def put(self, c):
    ...
    self.content[self.used] = c # Probably not necessary
    self.used += 1
  # end put
...
def ttm_cr(ttm, frame, br):  # Mark for creation
  ...
  if (crlen > 0):  # search only if possible success
    ...
    newbody = EMPTY
    while True:
      pos = body.find(crstring, offset)
      if (pos < 0):
        newbody += body[offset:]
        entry.body = newbody
        break
      else:
        newbody += body[offset:pos] + makecreatemark()
        offset = pos + crlen
# end ttm_cr
# end ttm_cr
...
def ttm_cs(ttm, frame, br):  # Call segment
  ...
  while p < len(entry.body):
    c = entry.body[p]
    if issegmark(c) or iscreatemark(c): break
    p += 1
  delta = (p - entry.residual)
  if (delta > 0):
    br.append(entry.body, entry.residual, delta)
...
def ttm_ttm_info_name(ttm, frame, br):
  for i in range(3, frame.argc()):
    ...
    if (not entry.builtin):
      ...
      for c in entry.body:
        if issegmark(c) or iscreatemark(c):
          sc = escapechar(c)
          br.append(sc)
        else:
          br.put(c)
...
# including the leading escape character if needed.
def escapechar(c, escapec=ESCAPE, controlEscapes=None):
  if controlEscapes == None: controlEscapes = EMPTY
  cc = u''
  if (iscreatemark(c)):
    return (u"^" + SEGFORMAT).format(CREATEMARK & INDEXMASK)
  elif (issegmark(c)) :
    return (u"^" + SEGFORMAT).format((ord(c) & INDEXMASK))
  elif (c == escapec):
    cc = escapec + escapec
  elif controlEscapes.find(c) >= 0:
...
def dbgprint(output, s, quote=None, escapec = '\\'):
  ...
  if (quote != None):
    ...
    ctlEscapes = u"\n\r\t\b\f"
  output.write(printablestring(s, escapec, ctlEscapes))
  if quote != None :
    output.write(quote)
  output.flush()
#end dbgprint
...
def printablestring(s, escapec, controlEscapes=None):
  ...
  for c in s:
    if (c == EOS): break
    c = escapechar(c,escapec,controlEscapes)
    ps += c    
  return ps
#end printablestring
